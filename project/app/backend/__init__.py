import logging, logging.config, datetime, time, requests, json, functools
...
logger_dealer_nl = configure_logger('default', 'access_dealer.log')


def run_time(func):
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        # logger_dealer_nl.debug('!---- Started: def {}:'.format(func.__name__))
        start = datetime.datetime.now()
        return_value = func(*args, **kwargs)
        # logger_dealer_nl.debug('----! Finished: def {}: in: {} seconds'.format(func.__name__,
        ...
        try:
            ...
            while count != 40:
                ...
                if data_response_status['code'] == '200':
                    if pause:
                        logger_dealer_nl.error(_msg_to_logger('PAUSE GOOD', data_response_status))
                    ...
                    break
                pause = True
                logger_dealer_nl.error(_msg_to_logger('PROBLEM', data_response_status))
...
from . import views
# from .main_dealer.spiders.zip_xml import XMLDownloader